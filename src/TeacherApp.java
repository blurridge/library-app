...
public class TeacherApp {
    ...
    private static String now = dtf.format(LocalDateTime.now());
    private static String extendDate = dtf.format(LocalDateTime.now().plusDays(7));
    private static String maxReturnDate = dtf.format(LocalDateTime.now().plusDays(30));
    ...
    public static int showMenu() throws InputMismatchException {
        ...
        System.out.println("[3]. Extend borrowing");
    ...
    public static void borrowMat() throws InputMismatchException {
        ...
        if(choice > 0 && choice <= matCount) {
            choice--;
            if(!readingMats[choice].isBorrowed) {
                input.nextLine();
                System.out.print("Enter name of teacher: ");
                String name = input.nextLine();
                readingMats[choice].setBorrowStatus(true, name, "Teacher", maxReturnDate);
    ...
    public static void extendBorrow() throws InputMismatchException {
        displayReadingMats();
        int choice = -1;
        System.out.println("Enter index of material to be extended: ");
        try {
            choice = input.nextInt();
        }
        catch (InputMismatchException e) {
